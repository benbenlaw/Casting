...
public class FuelRecipeCategory implements IRecipeCategory<FuelRecipe> {
    ...
    public FuelRecipeCategory(IGuiHelper helper) {
        ...
        this.icon = helper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(CastingBlocks.MULTIBLOCK_FUEL_TANK.get()));
    }
    ...
    public @NotNull Component getTitle() {
        return Component.translatable("gui.casting.jei.fuel");
    }
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, FuelRecipe recipe, IFocusGroup focusGroup) {
        ...
        for (int i = 0; i < mutableRecipes.size(); i++) {
            ...
            FuelRecipe coolant = mutableRecipes.get(i);

            builder.addSlot(RecipeIngredientRole.INPUT, slotX, slotY)
                    .addFluidStack(mutableRecipes.get(i).getFluid(), 1000)
                    .addRichTooltipCallback((slot, tooltip) ->
                            tooltip.add(Component.literal(coolant.fluid().getAmount() + "mB").withStyle(ChatFormatting.GOLD)))
                    .addRichTooltipCallback((slot, tooltip) ->
                            tooltip.add(Component.translatable("gui.casting.jei.fuel_temp", coolant.temp()).withStyle(ChatFormatting.GOLD)))
                    .addRichTooltipCallback((slot, tooltip) ->
                            tooltip.add(Component.translatable("gui.casting.jei.fuel_speed", coolant.duration()).withStyle(ChatFormatting.GOLD)))
